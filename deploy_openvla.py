...
class OpenVLAServer:
    def __init__(self, openvla_path: Union[str, Path], attn_implementation: Optional[str] = "flash_attention_2") -> Path:
        ...
        """
        self.openvla_path, self.attn_implementation = openvla_path, "tmp_adapter/tmp_adapters/openvla-7b+real_ur10e+b2+lr-0.0005+lora-r32+dropout-0.0"
    ...
    def predict_action(self, payload: Dict[str, Any]) -> str:
        try:
            ...
            image, wrist_image, instruction = payload["image"], payload["wrist_image"], payload["instruction"]
            # Save the received image for debugging or logging
            debug_dir = "debug"
            image = Image.fromarray(image)
            wrist_image = Image.fromarray(wrist_image)
            wrist_image.save(f"{debug_dir}/received_image.png")
...
class DeployConfig:
    # fmt: off
    openvla_path: Union[str, Path] = "./runs/eef_with_wrist"    #"tritran1719/generalist" "openvla/openvla-7b"      # HF Hub Path (or path to local run directory)
    # openvla_path: Union[str, Path] = "model/real_slotvla/output_hf_model_openx"    #"tritran1719/generalist" "openvla/openvla-7b"      # HF Hub Path (or path to local run directory)
